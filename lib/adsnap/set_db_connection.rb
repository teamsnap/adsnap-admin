...
module Adsnap
  module SetDbConnection
    ...
    def call
      database_configs = Adsnap::EncryptedConfig.find_by(key: Adsnap::EncryptedConfig::DATABASE_CREDENTIALS_KEY)
      ...
      if database_configs
        ...
        db_path = '/tmp/adsnap-admin-demo.sqlite3' unless File.exist?(db_path)

        File.write(db_path, Rails.root.join('adsnap-admin-demo.sqlite3').read) unless File.exist?(db_path)
