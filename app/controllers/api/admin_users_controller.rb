...
module Api
  class AdminUsersController < ApiBaseController
    ...
    load_and_authorize_resource class: 'Adsnap::AdminUser'
    ...
    def reset_password
      @admin_user = Adsnap::AdminUser.find(params[:admin_user_id])
    ...
    def build_admin_user
      @admin_user   = Adsnap::AdminUser.find_by(email: admin_user_params[:email])
      @admin_user ||= Adsnap::AdminUser.new(admin_user_params.merge(roles: [Adsnap::Role.admin]))
